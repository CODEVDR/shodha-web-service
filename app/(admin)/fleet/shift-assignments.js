...
const AssignmentCard = ({ assignment, onRelease }) => {
  ...
  // Display assignment date if available
  const assignmentDate =
    assignment.shiftDate ||
    assignment.startTimeIST?.date ||
    assignment.assignedAtIST?.date;
              ...
              <View className="flex-row items-center">
                ...
                {assignmentDate && (
                  <>
                    ...
                    >
                      {formatDisplayDate(assignmentDate)}
                    </Text>
                  </>
                )}
              </View>
...
export default function ShiftAssignments() {
  ...
  const handleAutoAssignInit = async () => {
    try {
      setLoading(true);
      const dateString = formatDateString(assignmentDate);
      const response =
        await AutoShiftService.initializeShiftAssignments(dateString);

      if (response.success) {
        Toast.show({
          type: "success",
          text1: "Success",
          text2: `Auto-assignments initialized for ${formatDisplayDate(dateString)}`,
        });
        setShowAutoAssignModal(false);
        setAssignmentDate(new Date()); // Reset to today
        loadData();
      } else {
        Toast.show({
          type: "error",
          text1: "Error",
          text2: response.message || "Failed to initialize assignments",
        });
      }
    } catch (error) {
      Toast.show({
        type: "error",
        text1: "Error",
        text2: error.message || "Failed to initialize assignments",
      });
    } finally {
      setLoading(false);
    }
        ...
        <View className="flex-row items-center justify-between">
          ...
          <View className="flex-row gap-2">
            <TouchableOpacity
              ...
              className="bg-green-600 p-3 rounded-full shadow-sm"
            >
          ...
          <View className="bg-white rounded-t-3xl p-6">
            ...
            <View className="bg-green-50 rounded-xl p-4 mb-6">
              <View className="flex-row items-start">
                ...
                >
                  Initialize automatic shift assignments for all drivers based
                  on rotation pattern. Select a date to schedule assignments.
                </Text>
              </View>
            </View>
            ...
            <View className="mb-6">
              ...
              >
                <View className="flex-row items-center">
                  ...
                  >
                    {formatDisplayDate(formatDateString(assignmentDate))}
                  </Text>
                </View>
                <Text
                  style={{
                ...
                >
                  {assignmentDate.toLocaleDateString("en-IN", {
                    day: "2-digit",
                    month: "short",
                    year: "numeric",
                  })}
