...
export default function EditTruck() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          permitType: truck.permitType || "State",
          permitDetails: truck.permitDetails || "",
          fitnessValidity: truck.fitnessValidity
            ? new Date(truck.fitnessValidity)
            : new Date(),
          insuranceValidity: truck.insuranceValidity
            ? new Date(truck.insuranceValidity)
            : new Date(),
          maintenanceSchedule: truck.maintenanceSchedule || "Quarterly",
          maintenancePeriod: truck.maintenancePeriod
            ? truck.maintenancePeriod.toString()
            : "90",
          fuelType: truck.fuelType || "Diesel",
          tankCapacity: truck.tankCapacity ? truck.tankCapacity.toString() : "",
          averageFuelConsumption: truck.averageFuelConsumption
            ? truck.averageFuelConsumption.toString()
            : "",
          odometer: truck.odometer ? truck.odometer.toString() : "",
          status: truck.status || "available",
          currentDriver: truck.currentDriver || null,
      ...
      Toast.show({
        type: "error",
        text1: "Error",
        text2: "Failed to load truck details",
        position: "top",
      });
    } finally {
      setFetching(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apiClient.put(`/trucks/${id}`, payload);

      if (response.success) {
        Toast.show({
          type: "success",
          text1: "Truck Updated",
          text2: "Truck details have been updated successfully",
          position: "top",
        });
        ...
        Toast.show({
          type: "error",
          text1: "Error",
          text2: response.message || "Failed to update truck",
          position: "top",
        });
      }
    } catch (error) {
      ...
      Toast.show({
        type: "error",
        text1: "Error",
        text2: error.message || "Failed to update truck. Please try again.",
        position: "top",
      });
    } finally {
      setLoading(false);
